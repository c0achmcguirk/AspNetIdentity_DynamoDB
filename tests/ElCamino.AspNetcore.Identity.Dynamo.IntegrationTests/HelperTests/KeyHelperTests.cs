...
namespace ElCamino.AspNet.Identity.Dynamo.Tests.HelperTests
{
    ...
    public class KeyHelperTests
    {
        ...
        public void Escape()
        {
            ...
            string escaped = KeyHelper.EscapeKey(url);
            Assert.IsFalse(escaped.Contains(ILLEGAL_CHAR), "Contains illegal char for row or partition key");

            Assert.IsNull(KeyHelper.EscapeKey(string.Empty), "Escape didn't return null with an empty string.");
        }
    }
}
